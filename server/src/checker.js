...
async function runCheck(store, opts) {
  const o = opts || {};
  const due = await store.dueForCheck(o.limit || 25, o.staleMs);
  const summary = { checked: 0, ok: 0, dead: 0, blocked: 0, error: 0 };

  for (const listing of due) {
    const result = await probe(listing.url, o);
    await store.recordCheck(listing.id, result.status, result.note);
    summary.checked++;
...
if (require.main === module) {
  ...
  createDb({ url: process.env.DATABASE_URL || undefined }).then((db) => {
